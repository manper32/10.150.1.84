...
select distinct ltrim(rtrim(indice)) deudor_id,	
				...
				MAX(horfingest) as Fecha_ultima_marcacion
from 
(
	select * 
...
group by ltrim(rtrim(indice)),ltrim(rtrim(telefono))
order by ltrim(rtrim(indice)) desc;
"""

#quey PostgeSQL delete
queryP_del_P = """
truncate table cbpo_colpatria.telefonos_positivos;
"""

#query insert PostgreSQL produccion
queryP_in_P ="""
INSERT INTO cbpo_colpatria.telefonos_positivos(
deudor_id
,telefono
,marcaciones
...
#cursorP_P.execute(queryP_in_P,anwr[37])
#conexionP_P.commit()
